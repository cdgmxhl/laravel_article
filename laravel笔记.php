...
//2.创建数据库
    //创建数据库 : 
        #1. 创建表文件 : 
            命令行执行 : php artisan make:migration create_article_table
        #2.修改up和down方法 :
            public function up(){
...
//7.使用软件包LaravelCollective/html替代原生html表单语法
    #1.安装软件包 : 
    ...
    #3.修改create.blade.php文件的form表单部分如下 : 
        {!! Form::model(null,['url'=>url('article'),'method'=>'post']) !!}
            ...
            {!! Form::button('提交',['class' => 'btn btn-primary']) !!}
        {!! Form::close() !!}
...
//8.修改文章
    //1.显示页面
        ...
        #2. 复制create.blade.php并重命名为edit.blade.php文件,修改内容如下:
            将     : {!! Form::model(null,['url'=>url('article'),'method'=>'post']) !!}
            修改为 : {!! Form::model($info,['url'=>url('article',['id'=>$info->id]),'method'=>'PUT']) !!}
    //2.验证数据及处理逻辑 : 
...
//9.删除文章
    ...
    //2.修改ArticleController的destroy方法 : 
...
//10. 注册
    ...
    #2. 新建LoginController.php
        命令行执行 : php artisan make:controller LoginController
    #3. LoginController里新增register方法如下 : 
        use App\User;
...
//12.显示用户信息及退出
    ...
    #2. article/index.blade.php中新增显示用户的数据
        <div>欢迎, {{Auth::user()->name}}  <a href="{{url('logout')}}">退出</a></div>
    #3. LoginController里新增logout方法如下 : 
...
//12.显示文章所属作者
    #1. Article.php文件添加如下方法 : 
        public function user(){
            return $this->belongsTo('App\User','user_id','id');
        }
    #2. 在index.blade.php中将包含'作者'的i标签修改为 : 
        <i>作者:{{$v->user->name}}</i>
...
//13.中间件验证用户是否登录
    #1. 创建中间件文件
        命令行执行 : php artisan make:middleware loginMid
    #2.修改loginMid.php : 
        use Illuminate\Support\Facades\Auth;
    ...
    #4.修改路由如下 : 
        ...
        # laravel5.4不能使用 Route::resource()->middleware('loginMid') 方式, 5.5可以